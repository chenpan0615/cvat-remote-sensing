...
class JobAnnotation:
    ...
    def _get_existing_shape_audit(self, shapes):
        def collect_shape_ids(items):
            ids = []
            for item in items:
                if item.get("id") is not None:
                    ids.append(item["id"])
                ids.extend(collect_shape_ids(item.get("elements", [])))
            return ids

        shape_ids = collect_shape_ids(shapes)
        if not shape_ids:
            return {}

        return {
            row["id"]: row
            for row in self.db_job.labeledshape_set.filter(pk__in=shape_ids).values(
                "id",
                "created_by_id",
    ...
    @staticmethod
    def _serialize_audit_date(value):
        return value.isoformat() if value else None

    def _prepare_shape_audit_fields(self, shape, existing_shape_audit, timestamp):
        existing_audit = existing_shape_audit.get(shape.get("id"))
        if existing_audit:
            created_by_id = existing_audit["created_by_id"]
            created_by = existing_audit["created_by__username"]
            created_date = existing_audit["created_date"]
            updated_by_id = self.user_id or existing_audit["updated_by_id"]
            updated_by = self.user_name if self.user_id else existing_audit["updated_by__username"]
            updated_date = timestamp if self.user_id else existing_audit["updated_date"]
        elif self.user_id:
        ...
        return {
            ...
            "created_date": self._serialize_audit_date(created_date),
            "updated_by": updated_by,
            "updated_date": self._serialize_audit_date(updated_date),
        }

    def _save_shapes_to_db(self, shapes, *, existing_shape_audit=None):
        existing_shape_audit = existing_shape_audit or {}
        audit_timestamp = timezone.now()
        audit_response_fields = {"created_by", "created_date", "updated_by", "updated_date"}
        ...
        def create_shapes(shapes, parent_shape=None):
            ...
            for shape in shapes:
                ...
                shape_audit_db_fields, shape_audit_response = self._prepare_shape_audit_fields(
                    shape,
                    existing_shape_audit,
                    audit_timestamp,
                )
                db_shape_data = {
                    key: value for key, value in shape.items() if key not in audit_response_fields
                }
                db_shape_data.update(shape_audit_db_fields)
                # FIXME: need to clamp points (be sure that all of them inside the image)
                # Should we check here or implement a validator?
                db_shape = models.LabeledShape(job=self.db_job, parent=parent_shape, **db_shape_data)
                ...
                shape.update(shape_audit_response)
    ...
    def _update_shapes(self, data):
        existing_shape_audit = self._get_existing_shape_audit(data["shapes"])
        for labeledshape_ids_chunk in take_by(
            [shape["id"] for shape in data["shapes"] if shape.get("id") is not None],
            chunk_size=1000,
        ):
            self._delete_job_labeledshapes(labeledshape_ids_chunk)
        self._save_shapes_to_db(data["shapes"], existing_shape_audit=existing_shape_audit)
    ...
    def update(self, data):
        ...
        self.reset()
        self._delete_job_labeledimages([image["id"] for image in data["tags"]])
        self._update_shapes(data)
        self._delete_job_labeledtracks([track["id"] for track in data["tracks"]])
        self._delete_job_intervals([interval["id"] for interval in data["intervals"]])
        self._save_tags_to_db(data["tags"])
        self._save_tracks_to_db(data["tracks"])
        self._save_intervals_to_db(data["intervals"])
...
def put_job_data(
    pk,
    data: AnnotationIR | dict,
    *,
    db_job: models.Job | None = None,
    user_id: int | None = None,
    user_name: str | None = None,
):
    annotation = JobAnnotation(pk, db_job=db_job, user_id=user_id, user_name=user_name)
    annotation.put(data)
...
@transaction.atomic
def put_task_data(pk, data, *, user_id: int | None = None, user_name: str | None = None):
    annotation = TaskAnnotation(pk, user_id=user_id, user_name=user_name)
    annotation.put(data)
...
@transaction.atomic
def patch_task_data(pk, data, action, *, user_id: int | None = None, user_name: str | None = None):
    annotation = TaskAnnotation(pk, user_id=user_id, user_name=user_name)
    if action == PatchAction.CREATE:
        annotation.create(data)
    elif action == PatchAction.UPDATE:
