...
def _convert_shape_audit(shape):
    def serialize_date(value):
        return value.isoformat() if value else None

    return OrderedDict(
        [
            ("created_by", shape.get("created_by__username")),
            ("created_date", serialize_date(shape.get("created_date"))),
            ("updated_by", shape.get("updated_by__username")),
            ("updated_date", serialize_date(shape.get("updated_date"))),
        ]
    )
...
class LabeledShapeSerializerFromDB(serializers.BaseSerializer):
    ...
    def to_representation(self, instance):
        def convert_shape(shape):
            ...
            result.update(_convert_shape_audit(shape))
