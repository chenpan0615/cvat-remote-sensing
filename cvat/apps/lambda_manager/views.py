...
class LambdaGateway:
    ...
    def _invoke_directly(self, func, payload):
        ...
        if os.path.exists("/.dockerenv"):  # inside a docker container
            namespace = settings.NUCLIO["FUNCTION_NAMESPACE"]
            url = f"http://nuclio-{namespace}-{func.id}:8080"
        else:
...
class LambdaJob:
    ...
    def _call_detector(
        ...
        *,
        ...
        collector = DetectionResultCollector(
            db_task,
            db_job,
            user_id=user_id,
            user_name=user_name,
